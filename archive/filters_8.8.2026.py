...
def get_sidebar_filters(
    ...
    """
    # Fehlende Daten robust behandeln
    if df_users is None:
    ...
    # ============================================================
    # Extra-Filter INITIALISIEREN (UNABHÄNGIG VOM DATUM!)
    # ============================================================
    ...
    if has_extra:
        with st.sidebar.expander("📊 Details (Ausleihe/Medien)", expanded=True):
            ...
            if enable_first_loan_toggle:
                ...
                first_loan_only = st.toggle("Ohne Verlängerungen", key=f"{prefix}_first_loan")
                st.divider()
    ...
    has_catalog = df_catalog is not None and not df_catalog.empty

    if has_catalog and catalog_filters_config:
        with st.sidebar.expander("📚 Medienbestand", expanded=False):
            for conf in catalog_filters_config:
                ...
                if col == "Datum der Aufnahme":
                    option = st.selectbox(
                        ...
                        key = f"{prefix}_aufnahme"
                    )
                    st.session_state[f"{prefix}_catalog_{col}"]= option
                    ...
                    key = f"{prefix}_catalog_{col}"

                    init_state_safe(key, values)
                    ...
                    catalog_filter_keys[col]=key

        catalog_filters = {}

        for col in catalog_filter_keys:

            catalog_filters[col] = st.session_state.get(
                f"{prefix}_catalog_{col}",
                []
            )

        catalog_filters["Datum der Aufnahme"] = st.session_state.get(
            f"{prefix}_catalog_Datum der Aufnahme",
            "Alle Medien"
        )
...
def build_filtered_data(data, filtered_users, filtered_loans, filter_state):
    ...
    extras_catalog = filter_state.get("catalog_filters", {})
    if extras_catalog.get('Lieferant'):
        df_books_all = df_books_all[
            df_books_all['Lieferant']
            .astype(str)
            .isin(extras_catalog['Lieferant'])
        ]
    if "Datum der Aufnahme" in df_books_all.columns:
        df_books_all['Datum der Aufnahme'] = pd.to_datetime(
            df_books_all['Datum der Aufnahme'],
            errors="coerce"
        )

        option = extras_catalog.get("Datum der Aufnahme")
        heute = pd.Timestamp.today()

        if option == "Aktuelles Jahr":
            df_books_all = df_books_all[
                df_books_all['Datum der Aufnahme'].dt.year
                == heute.year
            ]
        elif option == "Letzte 365 Tage":
            df_books_all = df_books_all[
                df_books_all['Datum der Aufnahme']
                >= heute - pd.Timedelta(days=365)
            ]
        elif option == "Letzte 2 Jahre":
            df_books_all = df_books_all[
                df_books_all['Datum der Aufnahme']
                >= heute - pd.timedelta(days=730)
            ]
